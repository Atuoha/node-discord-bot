...
a.channel.send("Hello there!");if(a.content.startsWith(PREFIX)){const [c,...e]=a.content.trim().substring(PREFIX.length).substring(/\s+/);if("kick"===c){if(!a.member.hasPermission("KICK_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>e.length)return a.channel.send("Please provide a User ID");const d=a.guide.cache.get(e[0]);d?d.kick().then(f=>f.channel.send(`${d} kicked user due to server rules and regulation violaition!!`)).catch(f=>a.channel.send("You do not have enough permission to kick out a user")):
...
value:"Returns a lucky number"},{name:"`$like`",value:"Likes the current message"},{name:"`$dislike`",value:"Dislikes the current message"});a.channel.send({embeds:[b]})}"$like"==a.content&&a.react("\ud83d\udc4d");"$rules"==a.content&&(a.react("\ud83d\udcaf"),b=(new MessageEmbed).setColor("#ffd046").setTitle("Server Rules and Regulations").setDescription("Welcome to MACC\u2019s Discord Community! We are\n          happy to have you and excited you\u2019re here.. But, before you get to meet our community and other special people like you, there are some important rules you\n          are expected to follow as part of our community.").addFields({name:"Safety and Harassment",
...
a.content.startsWith("$news")&&(a.react("\ud83d\udcaf"),b=encodeURI(`https://newsapi.org/v2/everything?q=crypto&apiKey=${process.env.NEWS}&pageSize=1&sortBy=publishedAt&language=en`),axios.get(b).then(c=>{const {title:e,source:{name:d},description:f,url:g}=c.articles[0];a.channel.send(`Latest news related to crypto:\n
            Title: ${e}\n
            Description:${f}\n
            Source: ${d}\n
            Link to full article: ${g}`)}).catch(c=>{a.channel.send("There was an error. Please try again later.")}));sadWords.some(c=>a.content.includes(c))&&(b=encouragements[Math.floor(Math.random()*encouragements.length)],a.react("\ud83d\ude0d"),a.reply(b));if(a.content.startsWith("$price"))if([,...b]=a.content.split(" "),2!==b.length)a.reply("You must provide the crypto and the currency to compare with!");else{const [c,e]=b;axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=${c}&vs_currencies=${e}`).then(d=>
{data[c][e]||a.reply("Please check your inputs. For example: $price bitcoin usd");a.react("\ud83d\ude0d");a.reply(`The current price of 1 ${c} = ${d[c][e]} ${e}`)}).catch(d=>{a.reply("Please check your inputs. For example: $price bitcoin usd")})}"$about"==a.content&&(a.react("\ud83d\udcc3"),a.channel.send("WELCOME TO THE MUTANT AGE CAMEL CLUB! \n          MACC is a collection of 10,000 Mutant Camel NFTs\u2014unique digital collectibles Roaming around the Ethereum blockchain. Your Mutant Camel is your membership card. Keep it safe, it grants you membership perks."));
