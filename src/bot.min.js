'use strict';"production"!==process.env.NODE_ENV&&require("dotenv").config();const {Client,Intents,MessageEmbed}=require("discord.js"),client=new Client({intents:[Intents.FLAGS.GUILDS,Intents.FLAGS.GUILD_MEMBERS,Intents.FLAGS.GUILD_MESSAGES]});client.on("ready",()=>{console.log(`Logged in as ${client.user.tag}!`)});function getRandomNumber(a,b){return Math.floor(Math.random()*(b-a)+a)}
function getQuote(){return fetch("https://zenquotes.io/api/random").then(a=>a.json()).then(a=>a[0].q+" -"+a[0].a)}function fetchPrice(a){return fetch(`https://min-api.cryptocompare.com/data/price?fsym=${a}&tsyms=USD`).then(b=>b.json()).then(b=>`Price: $${b[0].USD}`)}sadWords=["sad","depressed","unhappy","angry","miserable"];encouragements=["Cheer up!","Hang in there.","You are a great person"];const PREFIX="$";
sadWords.some(a=>msg.content.includes(a))&&msg.reply(encouragements[Math.floor(Math.random()*encouragements.length)]);
client.on("messageCreate",a=>{console.log(`[${a.author.username}]: ${a.content}`);if(!a.author.bot){"BTC PRICE"===a.content&&fetchPrice("BTC").then(c=>a.channel.send(c));"ETH PRICE"===a.content&&fetchPrice("ETH").then(c=>a.channel.send(c));"hi"===a.content.toLowerCase()||"hello"===a.content.toLowerCase()||"hey"===a.content.toLowerCase()?a.reply(`Hi ${a.author.username}! Welcome to Mutant-Age Camel Club. We happy to have you here with us. `):a.reply(`Hello ${a.author.username}! This is Mutant-Age Camel Club! We are happy to have you here`);
(a.content.toLowerCase().includes("hey bot")||a.content.toLowerCase().includes("general kenobi"))&&a.channel.send("Hello there!");if(a.content.startsWith(PREFIX)){const [c,...f]=a.content.trim().substring(PREFIX.length).substring(/\s+/);if("kick"===c){if(!a.member.hasPermission("KICK_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const d=a.guide.cache.get(f[0]);d?d.kick().then(e=>e.channel.send(`${d} kicked user due to server rules and regulation violaition!!`)).catch(e=>
a.channel.send("You do not have enough permission to kick out a user")):a.channel.send("No user with such ID here!")}if("ban"===c){if(!a.member.hasPermission("BAN_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const d=a.guide.cache.get(f[0]);d?d.ban().then(e=>e.channel.send(`${d} Banned user due to server rules and regulation violaition!!`)).catch(e=>a.channel.send("You do not have enough permission to ban out a user")):
a.channel.send("No user with such ID here!")}}if("$listCommands"==a.content){var b=(new MessageEmbed).setColor("#ffd046").setTitle("Server Commands").setDescription("Here you can see the list of the commands used on the server: ").addFields({name:"`$rules`",value:"Rules and Regulations"},{name:"`$about`",value:"About Server"},{name:"`$like`",value:"Likes the current message"},{name:"`$dislike`",value:"Dislikes the current message"},{name:"`$inspire`",value:"Inspirations"},{name:"`$lucky-number`",
value:"Returns a lucky number"});a.channel.send({embeds:[b]})}"$like"==a.content&&a.react("\ud83d\udc4d");"$inspire"===a.content&&getQuote().then(c=>a.channel.send(c));"$rules"==a.content&&(a.react("\ud83d\udcaf"),b=(new MessageEmbed).setColor("#ffd046").setTitle("Server Rules and Regulations").setDescription("Welcome to MACC\u2019s Discord Community! We are\n        happy to have you and excited you\u2019re here.. But, before you get to meet our community and other special people like you, there are some important rules you\n        are expected to follow as part of our community.").addFields({name:"Safety and Harassment",
