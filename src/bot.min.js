'use strict';"production"!==process.env.NODE_ENV&&require("dotenv").config();const token=process.env.TOKEN,id=process.env.ID,{Client,Intents,MessageEmbed}=require("discord.js"),client=new Client({intents:[Intents.FLAGS.GUILDS,Intents.FLAGS.GUILD_MEMBERS,Intents.FLAGS.GUILD_MESSAGES]});client.on("ready",()=>{console.log(`Logged in as ${client.user.tag}!`)});const PREFIX="$";
...
a.channel.send("Hello there!");if(a.content.startsWith(PREFIX)){const [e,...f]=a.content.trim().substring(PREFIX.length).substring(/\s+/);if("kick"===e){if(!a.member.hasPermission("KICK_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const c=a.guide.cache.get(f[0]);c?c.kick().then(d=>d.channel.send(`${c} kicked user due to server rules and regulation violaition!!`)).catch(d=>a.channel.send("You do not have enough permission to kick out a user")):
a.channel.send("No user with such ID here!")}if("ban"===e){if(!a.member.hasPermission("BAN_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const c=a.guide.cache.get(f[0]);c?c.ban().then(d=>d.channel.send(`${c} Banned user due to server rules and regulation violaition!!`)).catch(d=>a.channel.send("You do not have enough permission to ban out a user")):a.channel.send("No user with such ID here!")}}if("$listCommands"==
