'use strict';"production"!==process.env.NODE_ENV&&require("dotenv").config();const axios=require("axios"),{Client,Intents,MessageEmbed}=require("discord.js"),client=new Client({intents:[Intents.FLAGS.GUILDS,Intents.FLAGS.GUILD_MEMBERS,Intents.FLAGS.GUILD_MESSAGES]});client.on("ready",()=>{console.log(`Logged in as ${client.user.tag}!`)});function getRandomNumber(a,b){return Math.floor(Math.random()*(b-a)+a)}const PREFIX="$";
client.on("messageCreate",a=>{console.log(`[${a.author.username}]: ${a.content}`);if(!a.author.bot){"hi"===a.content.toLowerCase()||"hello"===a.content.toLowerCase()||"hey"===a.content.toLowerCase()?a.reply(`Hi ${a.author.username}! Welcome to Mutant-Age Camel Club. We happy to have you here with us. `):a.reply(`Hi ${a.author.username}! This is Mutant-Age Camel Club! We are happy to have you here`);(a.content.toLowerCase().includes("hey bot")||a.content.toLowerCase().includes("general kenobi"))&&
a.channel.send("Hello there!");if(a.content.startsWith(PREFIX)){const [d,...f]=a.content.trim().substring(PREFIX.length).substring(/\s+/);if("kick"===d){if(!a.member.hasPermission("KICK_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const c=a.guide.cache.get(f[0]);c?c.kick().then(e=>e.channel.send(`${c} kicked user due to server rules and regulation violaition!!`)).catch(e=>a.channel.send("You do not have enough permission to kick out a user")):
a.channel.send("No user with such ID here!")}if("ban"===d){if(!a.member.hasPermission("BAN_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const c=a.guide.cache.get(f[0]);c?c.ban().then(e=>e.channel.send(`${c} Banned user due to server rules and regulation violaition!!`)).catch(e=>a.channel.send("You do not have enough permission to ban out a user")):a.channel.send("No user with such ID here!")}}if("$listCommands"==
a.content){var b=(new MessageEmbed).setColor("#ffd046").setTitle("Server Commands").setDescription("Here you can see the list of the commands used on the server: ").addFields({name:"`$rules`",value:"Rules and Regulations"},{name:"`$about`",value:"About Server"},{name:"`$like`",value:"Likes the current message"},{name:"`$dislike`",value:"Dislikes the current message"},{name:"`$lucky-number`",value:"Returns a lucky number"});a.channel.send({embeds:[b]})}"$like"==a.content&&a.react("\ud83d\udc4d");"$rules"==
a.content&&(a.react("\ud83d\udcaf"),b=(new MessageEmbed).setColor("#ffd046").setTitle("Server Rules and Regulations").setDescription("Welcome to MACC\u2019s Discord Community! We are\n          happy to have you and excited you\u2019re here.. But, before you get to meet our community and other special people like you, there are some important rules you\n          are expected to follow as part of our community.").addFields({name:"Safety and Harassment",value:"This Discord server is a safe environment for all users and as such harassment is not allowed. Harassment includes, but is not limited to: offensive verbal or written comments about someone's race, religion, gender/gender identity, sexual orientation, physical appearance, and economic or social status. The use of offensive or discriminatory language is not permitted. Jokes or memes at the expense of anything listed above or bullying of any sort will not be tolerated. There are people here from many different\n              backgrounds so please be respectful of others.\n              The rules about harassment and bullying apply to both\n              text and voice channels. Additionally, harassing\n              community members via DM is not allowed and should be reported immediately. While interactions outside of our server are outside of our control, behavior which starts to affect the MACC Community will be dealt with\n              accordingly."},
...
{name:"Bots and commands ",value:"Do not abuse the bots and commands. Do not take advantage of the leveling system or participant within the community with bad faith. Staff have low tolerance of the offense. \n              Don\u2019t forget to have fun in the MACC community."}),a.channel.send({embeds:[b]}));a.content.startsWith("$news")&&(a.react("\ud83d\udcaf"),b=encodeURI(`https://newsapi.org/v2/everything?q=crypto&apiKey=${process.env.NEWS}&pageSize=1&sortBy=publishedAt&language=en`),axios.get(b).then(d=>
{const {title:f,source:{name:c},description:e,url:g}=d.data.articles[0];a.reply(`Latest news related to crypto:\n
            Title: ${f}\n
            Description:${e}\n
            Source: ${c}\n
            Link to full article: ${g}`)}).catch(d=>{a.channel.send("There was an error. Please try again later.")}));"$about"==a.content&&(a.react("\ud83d\udcc3"),a.channel.send("WELCOME TO THE MUTANT AGE CAMEL CLUB! \n          MACC is a collection of 10,000 Mutant Camel NFTs\u2014unique digital collectibles Roaming around the Ethereum blockchain. Your Mutant Camel is your membership card. Keep it safe, it grants you membership perks."));"$dislike"==a.content&&a.react("\ud83d\udc4e");"$lucky-number"==
a.content&&(a.react("\ud83d\ude0d"),b=getRandomNumber(0,1E3),a.reply(`Your new lucky number is ${b}.`))}});client.on("guildMemberAdd",a=>{const b=`Hey <@${a.id}>! Welcome to  Mutant-Age Camel Club! \n See commands list by typing: $listCommands`;a.guild.channels.fetch(process.env.ID).then(d=>{d.send(b)})});client.login(process.env.TOKEN);