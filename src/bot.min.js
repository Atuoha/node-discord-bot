...
a.channel.send("No user with such ID here!")}if("ban"===e){if(!a.member.hasPermission("BAN_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const c=a.guide.cache.get(f[0]);c?c.ban().then(d=>d.channel.send(`${c} Banned user due to server rules and regulation violaition!!`)).catch(d=>a.channel.send("You do not have enough permission to ban out a user")):a.channel.send("No user with such ID here!")}}if("$commands"==
