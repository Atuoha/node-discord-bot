...
function getQuote(){return fetch("https://zenquotes.io/api/random").then(a=>a.json()).then(a=>a[0].q+" -"+a[0].a)}sadWords=["sad","depressed","unhappy","angry","miserable"];encouragements=["Cheer up!","Hang in there.","You are a great person"];const PREFIX="$";sadWords.some(a=>msg.content.includes(a))&&msg.reply(encouragements[Math.floor(Math.random()*encouragements.length)]);
...
"$dislike"==a.content&&a.react("\ud83d\udc4e");"$lucky-number"==a.content&&(a.react("\ud83d\ude0d"),b=getRandomNumber(0,1E3),a.reply(`Your new lucky number is ${b}.`))}});client.on("guildMemberAdd",a=>{const b=`Hey <@${a.id}>! Welcome to Mutant-Age Camel Club! \n See commands list by typing: $listCommands. To get the price of BTC in USD type "BTC Price" and to get for ETH in USD type "ETH Price"`;a.guild.channels.fetch(process.env.ID).then(d=>{d.send(b)})});client.login(process.env.TOKEN);