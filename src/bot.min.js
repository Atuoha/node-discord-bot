...
a.channel.send("Hello there!");if(a.content.startsWith(PREFIX)){const [c,...f]=a.content.trim().substring(PREFIX.length).substring(/\s+/);if("kick"===c){if(!a.member.hasPermission("KICK_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const d=a.guide.cache.get(f[0]);d?d.kick().then(e=>e.channel.send(`${d} kicked user due to server rules and regulation violaition!!`)).catch(e=>a.channel.send("You do not have enough permission to kick out a user")):
a.channel.send("No user with such ID here!")}if("ban"===c){if(!a.member.hasPermission("BAN_MEMBERS"))return a.channel.send("You do not have enough permissions to use this command");if(1>f.length)return a.channel.send("Please provide a User ID");const d=a.guide.cache.get(f[0]);d?d.ban().then(e=>e.channel.send(`${d} Banned user due to server rules and regulation violaition!!`)).catch(e=>a.channel.send("You do not have enough permission to ban out a user")):a.channel.send("No user with such ID here!")}}if("$listCommands"==
...
a.content.startsWith("$news")&&(a.react("\ud83d\udcaf"),b=encodeURI(`https://newsapi.org/v2/everything?q=crypto&apiKey=${process.env.NEWS}&pageSize=1&sortBy=publishedAt&language=en`),axios.get(b).then(c=>{const {title:f,source:{name:d},description:e,url:g}=c.articles[0];a.channel.send(`Latest news related to crypto:\n
            Title: ${f}\n
            Description:${e}\n
            Source: ${d}\n
            Link to full article: ${g}`)}).catch(c=>{a.channel.send("There was an error. Please try again later.")}));sadWords.some(c=>a.content.includes(c))&&a.reply(encouragements[Math.floor(Math.random()*encouragements.length)]);"$about"==a.content&&(a.react("\ud83d\udcc3"),a.channel.send("WELCOME TO THE MUTANT AGE CAMEL CLUB! \n          MACC is a collection of 10,000 Mutant Camel NFTs\u2014unique digital collectibles Roaming around the Ethereum blockchain. Your Mutant Camel is your membership card. Keep it safe, it grants you membership perks."));
